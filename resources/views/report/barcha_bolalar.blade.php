...
@section('title', __('menu.barcha_bolalar') )
@section('content')
  <div class="pagetitle">
    <h1>{{ __('menu.barcha_bolalar') }}</h1>
    <nav>
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ route('home') }}">{{ __('menu.dashboard') }}</a></li>
        <li class="breadcrumb-item active">{{ __('menu.barcha_bolalar') }}</li>
      </ol>
    </nav>
  </div>
  <section class="section dashboard">
    <div class="card">
      <div class="card-body">
        <div class="row">
          <div class="col-6">
            <h5 class="card-title">{{ __('menu.barcha_bolalar') }}</h5>
          </div>
...
<script>
  function exportTableToExcel() {
      ...
      var fileName = "barcha_bolalar" + new Date().toISOString().slice(0, 10) + ".xlsx";
